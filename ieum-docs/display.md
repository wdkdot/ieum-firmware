...
- Applet 전환 뒤 5초 동안 reset과 rail 차단 없이 부분 갱신 세션 유지
...
Applet을 넘길 때는 첫 전환을 독립 실행 가능한 `DC` 부분 갱신으로 수행하고 패널 세션을 유지한다. 5초 안에 다음 Applet 전환이 들어오면 reset 없이 `1C`를 사용하고 유휴 시간을 다시 시작한다. 마지막 전환 뒤 5초가 지나면 추가 화면 갱신 없이 deep sleep, GPIO high-Z와 TPS22919 OFF를 수행한다.
...
Applet 전환은 `DC`로 clock과 analog를 준비한 뒤 5초 동안 같은 세션에서 `1C`를 이어서 사용한다. 유휴 종료는 화면을 다시 그리지 않고 컨트롤러 deep sleep과 rail 차단만 수행하므로, 세션을 닫기 위한 추가 전체 갱신이나 깜빡임은 발생하지 않는다.
